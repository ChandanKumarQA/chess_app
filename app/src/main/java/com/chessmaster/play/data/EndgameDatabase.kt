...
object EndgameDatabase {
    ...
    // === KING VS KING / OPPOSITION POSITIONS ===
    ...
    private val philidorPool = listOf(
        ...
        RawLesson("Philidor Defense (b-file)", "8/8/r7/1P6/1K6/8/8/1k6 w - - 0 1", "Third-rank defense on the b-file: push b6, Black rook retreats, White king moves to a5.", "Push b6, then Ka5.", listOf("b5b6", "a6a1", "b4a5"))
    )
    ...
    fun getLessons(category: String): List<EndgameLesson> = lessonCache.getOrPut(category) {
        ...
        (1..100).map { level ->
            ...
            // Ensure distinct titles per level without repetition
            EndgameLesson(
                id = "e_${catPrefix}_$level",
                title = "Level $level ($tier difficulty) • ${raw.subtitle}",
                category = category,
                fen = raw.fen,
