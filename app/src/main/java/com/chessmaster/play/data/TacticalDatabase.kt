...
object TacticalDatabase {
    ...
    private val doubleAttackPool = listOf(
        ...
        RawTacticalPuzzle("4k3/8/8/8/2r5/8/8/4Q1K1 w - - 0 1", listOf("e1e5", "e8d7", "e5c5"), "Queen Fork on King & Rook")
    )

    // 7. DISCOVERED ATTACK (8 bases)
    private val discoveredAttackPool = listOf(
        ...
        RawTacticalPuzzle("2q3k1/5ppp/8/8/8/3B4/5PPP/3R2K1 w - - 0 1", listOf("d3h7", "g8h8", "d1d8"), "Discovered Queen Invasion"),
        RawTacticalPuzzle("3r2k1/5ppp/8/8/8/3B4/5PPP/3R2K1 w - - 0 1", listOf("d3h7", "g8h7", "d1d8"), "Discovered Attack Winning Rook"),
        RawTacticalPuzzle("2q2k2/5ppp/8/8/8/2B5/5PPP/2R3K1 w - - 0 1", listOf("c3g7", "f8g7", "c1c8"), "Bishop Discovered Strike on Queen"),
    ...
    private val sacrificePool = listOf(
        ...
        RawTacticalPuzzle("r1b2rk1/pp3ppp/8/2qp4/8/3B1N2/PPP2PPP/R2QR1K1 w - - 0 1", listOf("d3h7", "g8h8", "f3g5"), "Kingside Bishop Sacrifice"),
    ...
    private val attractionPool = listOf(
        ...
        RawTacticalPuzzle("r1bqk2r/pppp1ppp/2n2n2/2b1p3/2B1P3/2N2N2/PPPP1PPP/R1BQK2R w KQkq - 0 1", listOf("c4f7", "e8f7", "f3e5"), "Attraction King to Exposed Square"),
        RawTacticalPuzzle("r1bqk2r/pppp1ppp/2n5/2b1p3/2B1P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 0 1", listOf("c4f7", "e8f7", "f3e5"), "Attraction Followed by Fork"),
        ...
        RawTacticalPuzzle("r1bqk2r/pp1p1ppp/2n2n2/2b1p3/2B1P3/2N2N2/PPPP1PPP/R1BQK2R w KQkq - 0 1", listOf("c4f7", "e8f7", "f3g5"), "Attraction Knight Outpost Strike"),
        RawTacticalPuzzle("r1bq1rk1/pppp1ppp/2n5/4p3/2B1n3/2N2N2/PPPP1PPP/R1BQR1K1 w - - 0 1", listOf("e1e4", "d7d5", "c4d5"), "Central Attraction Strike"),
        RawTacticalPuzzle("r1b2rk1/pp3ppp/2n5/2qp4/8/3B1N2/PPP2PPP/R2QR1K1 w - - 0 1", listOf("d3h7", "g8h7", "f3g5"), "Attraction of Black King to h7")
    )

    // 11. DEFLECTION (8 bases)
    private val deflectionPool = listOf(
        ...
        RawTacticalPuzzle("r5k1/5ppp/8/8/8/8/5PPP/1RR3K1 w - - 0 1", listOf("c1c8", "a8c8", "b1b8"), "Flank Deflection Mate"),
        RawTacticalPuzzle("2r3k1/5ppp/8/8/8/8/5PPP/3QR1K1 w - - 0 1", listOf("d1d8", "c8d8", "e1e8"), "Queen & Rook Deflection Combination"),
        RawTacticalPuzzle("6k1/6p1/5B2/8/8/8/5PPP/3Q2K1 w - - 0 1", listOf("d1d8", "g8f7", "d8e7"), "Deflection Infiltration Mate"),
        RawTacticalPuzzle("r5k1/5ppp/8/8/8/8/5PPP/3RR1K1 w - - 0 1", listOf("d1d8", "a8d8", "e1e8"), "Single Deflection on Back Rank"),
        RawTacticalPuzzle("r4rk1/5ppp/8/8/8/8/1Q3PPP/R5K1 w - - 0 1", listOf("a1a8", "f8a8", "b2b7"), "Deflection Infiltration on 7th Rank")
    )

    // 12. CLEARANCE (8 bases)
    ...
    fun getPuzzles(category: String): List<Puzzle> = categoryCache.getOrPut(category) {
        ...
        (1..100).map { i ->
            ...
            val baseIndex = (i - 1) % pool.size
            val varIndex = (i - 1) / pool.size
            val base = pool[baseIndex]
            var uniqueFen = PuzzleVariationHelper.getVariation(base.fen, base.solutionMoves, varIndex)
            if (uniqueFen in seenFens) {
                val variations = PuzzleVariationHelper.getUniqueVariations(base.fen, base.solutionMoves, 40)
